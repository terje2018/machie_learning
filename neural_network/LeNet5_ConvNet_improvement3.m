...
%
%该程序有错误，虽然跑出了较好的成绩。
%参考LeNet5_ConvNet_stop_early中的反响传播。
function [k1,B1,k2,B2,W,B3,W2,B4] = LeNet5_ConvNet_improvement3(mnist,k1,k2,W,W2,a)
    ...
    for i = 1:60000
        ...
        %(y_hat' * (1 - y_hat))这里写错了，一旦换成y_fc1，不太好训练。
        %因为两层都是logsig，初始化做不好，容易出现梯度爆炸或则消失。
        %虽然可以该程序不正确，但是依旧跑出了比较好的效果。
        DT_Y = (y_hat' * (1 - y_hat)) .* (W2'* DT_Y_FC2);
        DT_W = DT_Y*f';
        DT_B = DT_Y;
        DT_f = W' * DT_Y;
