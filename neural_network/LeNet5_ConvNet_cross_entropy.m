...
%
%
%由于5个模型有点难训练，所以先弄3个试试，猜想应该对结果有多提高。
%通过加入第三个模型，该模型正确率为0.9818。使用三个模型计算准确率，获得了0。9889的准确率。
%可以发现你这种方式的确可以提高准确率。
%这是因为每个模型通常都是一个局部最优解，通过多个局部最优解来逼近全局最优解。
%实际工程中，可以通过训练3+个模型来提高准确率。
%
%lenet的研究到此，下一步换个loss function，随后研究NIN。
%
%
%使用cross entropy做loss function，soft max分类。
%在几次训练过后，学习率逐步降到0。003，正确率稳定在0。983左右，最高可以达到0。9843。
%这说明使用cross entropy做损失函数，效果也不错。同时也可以简化计算。
function [k1,B1,k2,B2,W,B3,W2,B4] = LeNet5_ConvNet_cross_entropy(mnist,k1,B1,k2,B2,W,B3,W2,B4,a)
...
function SM = soft_max(W2, fc1, B4)
    M = W2 * fc1 + B4;
    [value] = max(M);
    SM = exp(M - value) / sum(exp(M - value));
end
